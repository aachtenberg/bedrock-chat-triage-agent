...
resource "aws_cloudfront_function" "auth" {
  ...
  publish = true
  comment = var.auth_mode == "basic" ? "Viewer-request HTTP Basic Auth." : "Viewer-request HMAC session validation."
  code = var.auth_mode == "basic" ? templatefile("${path.module}/cloudfront/basic-auth.js.tftpl", {
    base64_credentials = base64encode("${var.basic_auth_username}:${var.basic_auth_password}")
  }) : templatefile("${path.module}/cloudfront/auth.js.tftpl", {
